...
sub total_col {
   ...
   foreach (@dbretrows) {
      ...
      next if ($t=~/^ *$/);
      #if the column contains any data that is not an integer or a float, set N/A and quit
      if ($t!~/^\d+ *$|^\d+\.\d+ *$/) {
         $sum="N\\A";
         $t=0;
         last;
...
#call the print dialog form and send the sql results to a printer
sub printit {
   &savit("/tmp/dbgui.prt");
   my $p=$LW->PrintDialog(
      -Printers=>\@printers,
      -InFile=>'/tmp/dbgui.prt',
      -Program=>$psprint,
      );
   $p->Show;
   $outfile="";
